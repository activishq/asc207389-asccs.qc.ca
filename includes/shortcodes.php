...
if( !function_exists( 'activis_addshortcode_actcopy' ) ) :
	...
	function activis_addshortcode_actcopy( $atts ) {
		...
		ob_start();
			...
			printf(
				'<a href="https://activis.ca/?utm_source=Referal&utm_medium=Client&utm_campaign=%1$s" target="_blank" class="actcopy" title="%2$s Activis, %4$s">%2$s %3$s %4$s</a>',
				str_replace( 'www.', '', parse_url( site_url(), PHP_URL_HOST ) ),
				__( 'Conception Activis', 'activis-po' ), // Conception Activis
				...
				__( 'Votre agence web', 'activis-po' ) // Votre agence web
			);
